...
public class Translator {
    ...
    public static String clusterGet(String key) {
        return clusterGet(getLang(), key);
    }

    public static String clusterGet(Lang lang, String key) {
        if (StringUtils.isBlank(key)) {
            return StringUtils.EMPTY;
        }
        return translateKey(key, I18nService.lang(lang.getDesc().toLowerCase()));
    }

    public static Object clusterGets(Object keys) {
        return clusterGets(getLang(), keys);
    }

    public static Object clusterGets(Lang lang, Object keys) {
        Map<String, String> context = I18nService.lang(lang.getDesc().toLowerCase());
        return translateObject(keys, context);
    }
    ...
    private static Lang getLang(HttpServletRequest request) {
        ...
        try {
            ...
            if (request != null) {
                ...
                if (StringUtils.isNotBlank(request.getHeader(HttpHeaders.ACCEPT_LANGUAGE))) {
                    ...
                    if (Lang.getLangWithoutDefault(acceptLanguage) != null) {
                        if (ApiKeyHandler.isApiKeyCall(request)) {
                            // if it's api call and Accept-Language header is set
                            return Lang.getLang(acceptLanguage);
                        }
                ...
                if (GlobalConfigurations.getProperty(("shiro.enabled"), boolean.class, true)) {
                    // Shiro is disabled in some modules, e.g. gateway.
                    if (SessionUtils.getUser() != null && StringUtils.isNotBlank(SessionUtils.getUser().getLang())) {
                        preferLang = SessionUtils.getUser().getLang();
                    }
                }
    ...
    private static String getSystemParameterLanguage(String defaultLang) {
        ...
        try {
            ...
            String systemLanguage = CommonBeanFactory.getBean(ParameterCommonService.class).getSystemLanguage();
    ...
    private static Object translateObject(Object javaObject, final Map<String, String> context) {
        ...
        try {
            ...
            if (javaObject instanceof springfox.documentation.spring.web.json.Json) {
                springfox.documentation.spring.web.json.Json json = (springfox.documentation.spring.web.json.Json) javaObject;
                return new springfox.documentation.spring.web.json.Json(translateRawString(null, json.value(), context).toString());
            }
